...
**池回收边界（P3-07 实施定夺）**：集群引擎恒无等待项（D9），Semaphore 条目随持有清零即回收——Leader 内存队列的在队者对此不可见。后果与裁决：池清空后队首重发若携带 `total` 断言则自动重建同规模池并授予（等待者位次保持，先到先得序不变，与"池重置为满"不可区分）；**纯加入者（total=0）重发将被显式拒绝（INVALID_REQUEST）**，由客户端以带断言句柄重新获取。单机无此分叉（等待者在条目内保池）。此为集群形态下的许可族生命周期约定，测试矩阵两分支均钉住。

### D5 Latch 生命周期：归零即空、随框架回收

归零瞬间对全部 awaiter 广播（逐个 `listener.notifyHead`，接口零改动）；广播不离队——awaiter 各自重发 await 命中"已归零"规则时离队。条目一经定型**永久存续**（isEmpty 恒 false）：未归零屏障等待扣减的载体必须存活，归零屏障以条目存续承载一次性护栏与晚到者放行（P3-07 实施修订——参与者驱动的回收会在归零后放行与"新 total 重建"之间引入时序歧义，且集群影子镜像与引擎 GC 竞态无法在不引入水位协议的前提下对齐）。泄漏治理走约定：每轮屏障使用带轮次 id 的 key（examples/文档明示），条目本体极小（计数+参与集）。（Phase 3 后如需按留存期回收，§9 遗留项评估——须连带快照与集群镜像策略一并设计。）Latch 无租约：`LatchEntry.leaseToken()` 恒 0、永不入到期堆（`CoreEngine.expireDue` 实际因不入堆而零分支成本）。
