...
- [x] 4.2 断言与证据：恢复 < 10s；双场景——home=死主（失锁回调触发，§8 行 1）/ home=存活 Follower（续租不断、failover 后释放成功，§8 行 2）；等待者重排队最终获授；不变式检查器（同 key 至多一写持有；停载+一租约期后锁表空）输出结构化演练报告入库 `docs/`
- [x] 4.3 详设 §11 验收 1/2 证据收集：演练报告 + 组 2/3 IT 结果汇总为 S3 退出检查单
...
- [x] 5.3 `mvn -s /home/lam/repo/settings.xml clean verify` 全绿（默认构建不含 drill）+ `-Pdrill` 演练通过 → S3 退出

## 开放项

- **P2-14 进程级演练 <10s 计时门（4.2 / 4.3 / 5.3）：已关闭（2026-08-31 本机复核）**。为演练补诊断插桩（杀点前三节点角色快照、子进程日志落 `target/drill-logs/`、报告入库仓库根 `docs/`）后，全量演练（A+B 同 fork）连跑 4/4 通过；场景 A 有效样本 7 个实测 1621–1806ms（判定线 10s 余量 ≥5.5 倍），角色快照证实杀点目标为自报 Leader。此前失败记录集中于 `mvn` reactor 刚退出的热机瞬态（load≈2.6，插桩后未复现）。崩溃选举对 CPU 争抢敏感维持原结论：**发布级复核建议独占硬件**（用户文档 §5 口径同步为部署建议）；确定性语义由 `ClientClusterIT`（in-JVM 真 3 节点：home-kill 恢复、存活让位锁不丢、隔离判丢、等待重排）+ `LeaderFailoverServerTest`（选举空窗/提示跟随/转发车道/三源一致）默认构建全绿兜底。证据：`docs/failover-drill-2026-08-31.md`、变更域 `s3-exit-checklist.md`。
