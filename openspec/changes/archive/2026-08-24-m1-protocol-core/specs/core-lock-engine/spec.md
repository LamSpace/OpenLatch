...
对可重入锁类型（REENTRANT、READ、WRITE 的写侧），同一归属再次获取 MUST 使持有计数加一并返回成功，且租约 MUST 刷新为整段新租期（沿用原凭证）。不可重入类型（SIMPLE）的同归属再次获取 MUST 按普通竞争处理（排队等待，直至让出或租约到期）。
