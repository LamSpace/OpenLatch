...
| 5 | Ratis 3.3.0 leader 复制停摆：重启归群语境下，新 leader 任期起始条目可能永不提交（双稳态），写面 `LeaderNotReady` 阻塞不自愈 | **根因收口 + 产品侧自愈承载（v1.6，`phase2-leader-stall-followup`）**：①机理——`GrpcLogAppender` INCONSISTENCY 回退被 `request==null` keep-nextIndex 分支与不可唤醒 `sleepForErrors` 抵消成自持环，任期 conf 条目（`startupLogEntry`）永不到多数派中位（取证与三候选判定见 `openspec/changes/phase2-leader-stall-followup/observations-leader-stall-rootcause.md`；前件含产品配置 `purge.upto.snapshot.index` × 库 `trigger-when-stop` 的复合）。②自愈——`cluster-node-lifecycle` 新增"复制停滞自愈"需求：Leader 任期 `T_stall=max(10s,5×election)` 内 commitIndex 连续 M=3 采样零推进 → 让位最新对侧一次 → 观察窗仍冻结升级进程退出（退出码 1，交外部 supervisor；5 分钟冷却）。③口径修正——重放实测**两序皆可停**（14 测试 3 命中，含先从后主 1 次），"先从不先主"推荐序降级为降概率手段（部署文档同步）；`-Pdrill` 滚动演练判据改分段（自愈预算窗后零残留）。④上游——3.3.1 未发布且 tag 后相关路径零变更（升级不根治）；提报经定夺**不投递**（成稿与取证包归档备查 `upstream-ratis-issue-draft.md`、`evidence-r5-jstack/`），B 形态跟踪由升级评估 changelog 对账承载。原始收口档案 `openspec/changes/phase2-release-closure/defects/leader-replication-stall-ratis-3.3.0.md` 状态已更新 |
