...
| 依据     | 《OpenLatch-Phase2-详细设计说明书》v1.0 §2/§13.1；`openspec/changes/archive/2026-08-30-phase2-s1-raft-poc` |
| 证据     | PoC 工作树已归档退役（2026-09-12）：`git checkout archive/raft-selection-poc` 可完整取回（代码、`run-matrix.sh`、原始 results JSON、摩擦与冻结文档） |

> 本报告全部数字可由归档树内 `python3 poc/raft-selection/summarize.py` 从
> `poc/raft-selection/results/<candidate>-<exp>-<round>.json` 复现；
> 实验公平协议与对称调整登记见归档树内 `config-freeze.md`。
> 复现步骤见文末附录；`design D7` 等内部代号仅存于归档区，叙述以本文件为准。
...
## 附录：证据与复现（归档树锚定，2026-09-12 起）

PoC 数据与工作树保存于 git tag **`archive/raft-selection-poc`**（主干已删除，历史可取回）：

| 证据 | 归档树内路径 |
|---|---|
| 原始数据（21 轮正式 + round-0 开发轮 + 3.3.0 冒烟） | `poc/raft-selection/results/*.json` |
| 实验矩阵脚本 | `poc/raft-selection/run-matrix.sh` |
...
| 零改动核验 | 归档树内 `git diff`（主干 tracked 文件）+ PoC 独立 reactor 不入根 `<modules>` |

```bash
git clone <repo> openlatch-poc && cd openlatch-poc
git checkout archive/raft-selection-poc
mvn -s <settings> -pl openlatch-core,openlatch-protocol -am install -DskipTests
mvn -s <settings> -f poc/raft-selection/pom.xml install
