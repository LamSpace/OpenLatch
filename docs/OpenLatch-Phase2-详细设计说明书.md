...
概要设计 §5.3 指定候选： **Apache Ratis** 与 **SOFAJRaft**。不预先锁定，本节给出评估框架，S1 PoC 后定案并将结论回写本文档修订版。
...
4. `CoreEngine` 注入的 `Clock` 在集群模式下使用"条目携带时刻"回放：回放 `LOCK_ACQUIRE_ENTRY` 时，租约到期 = 条目内记录的授予时刻 + 租期，而非回放时的物理时钟。实现上为 `CoreEngine` 增加一个命令级时间覆盖参数（core 的唯一增量改动）。
...
| 4 | Raft 库与 Netty 的线程/内存共存细节依赖 S1 PoC 结论                   | PoC 任务书 §2.3 第 5 项覆盖                                |
...
| ID    | 子任务             | 内容与交付物                                                                                                 | 前置         | 验证                                  |
|-------|--------------------|--------------------------------------------------------------------------------------------------------------|--------------|---------------------------------------|
| P2-01 | PoC 环境搭建       | 本机 3 节点最小集群骨架（两库共用负载脚本与计时工具）                                                        | Phase 1 发布 | 任一库可组集群并完成选主              |
| P2-02 | Ratis 接入原型     | `CoreEngine` 挂入 Ratis 状态机；单键授予/释放走复制                                                          | P2-01        | 功能跑通；记录接入改动量              |
| P2-03 | SOFAJRaft 接入原型 | 同上范围，JRaft 侧实现                                                                                       | P2-01        | 功能跑通；记录接入改动量              |
| P2-04 | 对比评估与选型定案 | 按 §2.4 门槛逐项实测：授予延迟 P99、吞吐、选主计时、快照恢复耗时、API 侵入度；输出选型报告并回写本文档修订版 | P2-02、P2-03 | 报告含逐项数据；通过评审；**S1 退出** |
