...
public final class LockEntry implements KeyEntry {
    ...
    public synchronized CoreInspection.KeySnapshot snapshot(long now) {
        List<CoreInspection.HolderSnapshot> holderSnaps = new ArrayList<>();
        if (writer != null) {
            holderSnaps.add(new CoreInspection.HolderSnapshot(
                    writer.sessionId(), writer.threadId(), writeCount,
                    CoreInspection.HolderRole.WRITER));
        }
        for (Map.Entry<Owner, Integer> en : readers.entrySet()) {
            holderSnaps.add(new CoreInspection.HolderSnapshot(
                    en.getKey().sessionId(), en.getKey().threadId(), en.getValue(),
                    CoreInspection.HolderRole.READER));
        }
